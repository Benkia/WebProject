...
{
    public class FansController : AbstractController
    {
        ...
        public ActionResult Create()
        {
            ...
            //if (user.FindFirst("FanID") != null && user.FindFirst("FanID").Value != 0)
            //{
        ...
        public ActionResult Create([Bind(Include = "ID,FirstName,LastName,Gender,BirthDay,PazamInClub,Address")] Fan fan)
        {
            if (ModelState.IsValid)
            {
                ...
                //ApplicationUser user = db.Users.First(usr => usr.UserName.Equals(userName));
                fan.UserID = user.Id;

                db.fans.Add(fan);
                db.SaveChanges();

                var fanIdClaim = (User.Identity as ClaimsIdentity).FindFirst("FanID");
                if (fanIdClaim != null)
                    (User.Identity as ClaimsIdentity).RemoveClaim(fanIdClaim);

                (User.Identity as ClaimsIdentity).AddClaim(new Claim("FanID", fan.ID.ToString()));
